...
GFxValue::GFxValue(const GFxValue& src) : objectInterface(nullptr), type(src.type)
{
	data = src.data;
	if (src.IsManaged() && src.objectInterface && src.data.obj)
	{
...
const GFxValue& GFxValue::operator = (const GFxValue& src)
{
	if (this != &src)
	{
		CleanManaged();
...
const GFxValue& GFxValue::operator = (const GFxValue * pSrc)
{
	if (this != pSrc)
	{
		CleanManaged();
		type = pSrc->type;
		data = pSrc->data;
		if (pSrc->IsManaged() && pSrc->objectInterface && pSrc->data.obj)
		{
			objectInterface = pSrc->objectInterface;
			AddManaged();
		}
	}
	return *this;
